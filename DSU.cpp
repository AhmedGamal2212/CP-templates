...
struct station{
    int dist, cost, t, freq, st;

    station(){
        dist = cost = t = freq = st = 0;
    }
    station(int dist, int cost, int t, int freq, int st){
        this -> dist = dist;
        this -> cost = cost;
        this -> t = t;
        this -> freq = freq;
...
ll dp(int idx, ll curr){
    if(idx >= n){
        return 0;
    }

    ll& ret = memo[idx][curr];
    if(~ret)
        return ret;

    ret = dp(idx + 1, curr);
    if(curr + price[idx] <= x)
        updmax(ret, pages[idx] + dp(idx + 1, curr + price[idx]));
